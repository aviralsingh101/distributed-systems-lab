...
export function buildMetricsConfig(topicId, title) {
  ...
  const OVERRIDES = {
    "retry-storm": {
      drawMode: "retry-fanout",
      fanoutKind: "retry",
      fanoutClients: 5,
      fanoutRetries: 3,
      note: "Each client retries on timeout — N clients × R retries multiplies load on a degraded backend. Toggle burst for sustained retry storm.",
      burstLabel: "Retry storm (all clients retrying)",
      capacityDefault: 18,
      refillDefault: 2,
      capacityLabel: "Backend capacity",
      refillLabel: "Recovery rate",
      bucketLabel: "Backend headroom",
      sendActionLabel: "Trigger retry wave",
      acceptedLabel: "SERVED",
      droppedLabel: "TIMEOUT",
      acceptedSeriesLabel: "Served req/s",
      droppedSeriesLabel: "Timed out req/s",
      chartTitle: "Retry storm outcomes (req/s)",
      droppedAlertLabel: "BACKEND OVERLOAD — RETRY STORM",
      fanoutHint: "Clients retry in parallel when backend slows down",
      fanoutNodePrefix: "C",
      originTitle: "Payment API",
    },
    "thundering-herd": {
      drawMode: "cache-stampede",
      fanoutKind: "cache-miss",
      fanoutClients: 6,
      note: "Cache entry expires — every client wakes at once and hits the database. Toggle burst to simulate TTL expiry stampede.",
      burstLabel: "Cache expired (TTL)",
      capacityDefault: herdCapacityDefault,
      refillDefault: herdRefillDefault,
      capacityLabel: "DB capacity",
      refillLabel: "Query drain rate",
      bucketLabel: "DB headroom",
      sendActionLabel: "Cache miss",
      acceptedLabel: "SERVED",
      droppedLabel: "DB TIMEOUT",
      acceptedSeriesLabel: "DB queries/s",
      droppedSeriesLabel: "Timed out/s",
      chartTitle: "Thundering herd on DB (queries/s)",
      droppedAlertLabel: "DB OVERLOAD — THUNDERING HERD",
      fanoutHint: "TTL expires → every client misses cache together",
      fanoutNodePrefix: "App",
      cacheTitle: "Redis cache",
      originTitle: "Ledger DB",
    },
    "cache-stampede": {
      drawMode: "cache-stampede",
      fanoutKind: "cache-miss",
      fanoutClients: 5,
      note: `Cache miss triggers parallel origin fetches. ${stampedeVisualHint} Use single-flight / request coalescing to collapse to one fetch.`,
      ...
      fanoutHint: "Many clients → parallel origin fetch (no coalescing)",
      fanoutNodePrefix: "C",
      cacheTitle: "Cache",
      originTitle: "Origin API",
    },
    dogpile: {
      drawMode: "cache-stampede",
      fanoutKind: "cache-miss",
      fanoutClients: 5,
      note: `Many workers detect the same cache miss and recompute in parallel. ${stampedeVisualHint} Lock around refresh or single-flight dedupes the work.`,
      burstLabel: "Dogpile on miss",
      capacityDefault: stampedeCapacityDefault,
      refillDefault: stampedeRefillDefault,
      capacityLabel: "Origin capacity",
      refillLabel: "Recompute drain",
      bucketLabel: "Origin headroom",
      acceptedLabel: "COMPUTED",
      droppedLabel: "OVERLOAD",
      sendActionLabel: "Cache miss",
      acceptedSeriesLabel: "Origin recomputes/s",
      droppedSeriesLabel: "Overloaded req/s",
      chartTitle: "Dogpile origin pressure (req/s)",
      droppedAlertLabel: "ORIGIN OVERLOAD — PARALLEL RECOMPUTE",
      fanoutHint: "Workers miss cache → all recompute together",
      fanoutNodePrefix: "W",
      cacheTitle: "Shared cache",
      originTitle: "Recompute svc",
    },
    "hot-key": {
    ...
    "retry-amplification": {
      drawMode: "retry-chain",
      fanoutKind: "chain",
      fanoutClients: 4,
      fanoutRetries: 2,
      chainDepth: 2,
      note: "Each tier retries on timeout — load multiplies down the call chain (clients × retries × hops).",
      burstLabel: "Retry storm across tiers",
      capacityDefault: stampedeCapacityDefault,
      refillDefault: stampedeRefillDefault,
      capacityLabel: "Leaf service capacity",
      refillLabel: "Recovery rate",
      bucketLabel: "Downstream headroom",
      sendActionLabel: "Trigger retry chain",
      acceptedLabel: "SERVED",
      droppedLabel: "OVERLOAD",
      acceptedSeriesLabel: "Served req/s",
      droppedSeriesLabel: "Overloaded req/s",
      chartTitle: "Retry amplification pressure (req/s)",
      droppedAlertLabel: "LEAF OVERLOAD — RETRIES MULTIPLY",
      fanoutHint: "N clients × R retries × service hops → leaf DB",
      fanoutNodePrefix: "C",
      originTitle: "Ledger DB",
    },
    "coordinated-omission": {
      mode: "latency",
      drawMode: "coord-omission",
      note: "Closed-loop load generators wait for each response before sending the next — benchmark hides stalls and reports artificially low p99.",
      burstLabel: "Backend stall (hidden)",
      capacityDefault: 100,
      capacityMin: 50,
      capacityMax: 250,
      refillDefault: 30,
  ...
  return {
    ...
    fanoutClients: o.fanoutClients,
    fanoutRetries: o.fanoutRetries,
    fanoutKind: o.fanoutKind,
    chainDepth: o.chainDepth,
    originTitle: o.originTitle,
    cacheTitle: o.cacheTitle,
  };
}
