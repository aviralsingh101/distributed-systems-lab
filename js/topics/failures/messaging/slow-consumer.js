...
export const meta = { id: "slow-consumer", title: "Slow Consumer", category: "messaging" };

export const content = {
  oneliner: `Lag grows without bound.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>Lag grows without bound. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>Slow Consumer</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Slow Consumer</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Slow Consumer</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Slow Consumer timeline">
<defs><marker id="fig-slow-consumer-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
</svg>`, caption: `How Slow Consumer unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Slow Consumer in request path">
<defs><marker id="fig-slow-consumer-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Slow Consumer</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-slow-consumer-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Slow Consumer</text>
</svg>`, caption: `Slow Consumer on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return mountSimulation(stage, panel, stageEl, {
    note: "Arrival rate vs processing throughput → lag.",
    params: [{ key: "arr", label: "Arrival rate", min: 40, max: 160, step: 10, value: 120, unit: "/s", live: true }],
    toggles: [{ key: "fix", label: "Add a second consumer", kind: "ok", value: false }],
    frame(ctx, t, dt) {
      const d = ctx.d; const fix = ctx.toggles.fix; const arr = ctx.params.arr;
      const perConsumer = 70; const cap = perConsumer * (fix ? 2 : 1);
      const net = arr - cap;
      if (ctx.state.lag === undefined) ctx.state.lag = 20;
      if (dt > 0) ctx.state.lag = clamp(ctx.state.lag + net * dt * 1.8, 0, 500);
      const lag = ctx.state.lag;
      d.node(80, 230, 120, 56, { title: "Stream", color: C.queue, active: true, value: arr + "/s in" });
      d.node(760, 180, 160, 52, { title: "Consumer 1", color: C.service, active: true, value: perConsumer + "/s" });
      if (fix) d.node(760, 270, 160, 52, { title: "Consumer 2", color: C.ledger, active: true, value: perConsumer + "/s" });
      d.gauge(280, 430, 440, 18, clamp(lag / 500), { color: net > 0 ? C.err : C.ok, label: "consumer lag", value: Math.round(lag) + " msgs" });
      d.text(500, 480, net > 0 ? "lag rising " + net + "/s" : "lag draining " + (-net) + "/s", { size: 12, align: "center", color: net > 0 ? C.err : C.ok });
      ctx.setStatus(net > 0 ? "consumer too slow — lag grows" : "throughput > arrivals — catching up", net > 0 ? "err" : "ok");
    },
  });
}
