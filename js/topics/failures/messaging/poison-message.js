...
export const meta = { id: "poison-message", title: "Poison Message", category: "messaging" };

export const content = {
  oneliner: `Consumer always crashes on it.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>Consumer always crashes on it. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>Poison Message</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Poison Message</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Poison Message</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Poison Message timeline">
<defs><marker id="fig-poison-message-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
</svg>`, caption: `How Poison Message unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Poison Message in request path">
<defs><marker id="fig-poison-message-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Poison Message</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-poison-message-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Poison Message</text>
</svg>`, caption: `Poison Message on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return mountSimulation(stage, panel, stageEl, {
    note: "One message crashes the consumer every time.",
    toggles: [{ key: "fix", label: "DLQ after 3 attempts", kind: "ok", value: false }],
    frame(ctx, t) {
      const d = ctx.d; const fix = ctx.toggles.fix;
      const cons = { x: 780, y: 200 };
      const ph = phaseOf(t, [1, 1, 1, 1.4]);
      const attempts = fix ? Math.min(3, ph.i + 1) : (ph.loops * 4 + ph.i) % 9 + 1;
      const parked = fix && ph.i >= 3;
      // queue
      const msgs = ["poison", "m2", "m3", "m4"];
      msgs.forEach((m, i) => {
        const x = 150 + i * 120;
        const isPoison = i === 0;
        const gone = isPoison && parked;
        const blocked = !isPoison && !parked;
        d.node(x - 52, 200, 104, 52, { title: m, color: isPoison ? C.err : C.service, state: gone ? "dim" : (blocked ? "warn" : ""), value: isPoison && !gone ? "try " + attempts : (blocked ? "blocked" : "") });
      });
      d.node(cons.x - 60, cons.y - 26, 120, 52, { title: "Consumer", color: C.ledger, state: (!parked) ? "err" : "ok", value: parked ? "processing m2✓" : "crash!" });
      // DLQ
      if (fix) d.node(150 - 52, 380, 104, 52, { title: "DLQ", color: C.warn, state: parked ? "warn" : "dim", value: parked ? "poison" : "" });
      if (parked) d.arrow(150, 252, 150, 380, { color: C.warn, width: 2, label: "parked", progress: 1 });
      ctx.setStatus(fix ? (parked ? "poison in DLQ — queue flows again" : "attempt " + attempts + "/3…") : "queue blocked — poison redelivered forever", fix && parked ? "ok" : "err");
    },
  });
}
