...
export const meta = { id: "n-plus-one", title: "N+1 Query Problem", category: "prod-eng" };

export const content = {
  oneliner: `One query becomes N+1.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>One query becomes N+1. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>N+1 Query Problem</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
<p><b>N+1 Query Problem</b> affects how concurrent payment requests interact with Wallet, Order Service, Gateway, and Ledger under production load — not just in single-threaded dev environments.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>N+1 Query Problem</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>N+1 Query Problem</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="N+1 Query Problem timeline">
<defs><marker id="fig-n-plus-one-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
</svg>`, caption: `How N+1 Query Problem unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="N+1 Query Problem in request path">
<defs><marker id="fig-n-plus-one-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">N+1 Query Pro…</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-n-plus-one-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — N+1 Query Problem</text>
</svg>`, caption: `N+1 Query Problem on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  ...
  return mountSimulation(stage, panel, stageEl, {
    note: "Load N payments, then their customers.",
    toggles: [{ key: "fix", label: "Batch with a single IN query", kind: "ok", value: false }],
    frame(ctx, t) {
      const d = ctx.d; const fix = ctx.toggles.fix;
      const app = { x: 200, y: 260 }, db = { x: 800, y: 260 };
      d.node(app.x - 80, app.y - 34, 160, 68, { title: "App / ORM", color: C.service, active: true });
      d.node(db.x - 80, db.y - 34, 160, 68, { title: "Database", color: C.ledger, state: (!fix) ? "warn" : "ok", active: true });
      const queries = fix ? 2 : 1 + N;
      // animate query tokens
      for (let i = 0; i < queries; i++) {
        const p = cycle(t * 0.7 + i / queries, 1);
        const y = app.y - 30 + (i / Math.max(1, queries - 1)) * 60;
        const pos = d.along(app.x + 80, y, db.x - 80, y, p);
        d.token(pos.x, pos.y, { r: 6, color: (i === 0 || fix) ? C.ok : C.warn, glow: false });
      }
      d.text(500, 150, (fix ? "1 list query + 1 IN query" : "1 list query + " + N + " per-row queries"), { size: 13, align: "center", color: C.muted });
      d.badge(500, 400, queries + (queries === 1 ? " query" : " queries total"), { color: fix ? C.ok : C.err, filled: !fix, align: "center" });
      ctx.setStatus(fix ? "2 queries (batched) — flat with N" : (1 + N) + " queries — scales with N", fix ? "ok" : "err");
    },
  });
}
