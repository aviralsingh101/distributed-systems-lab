...
export const meta = { id: "noisy-neighbor", title: "Noisy Neighbor", category: "prod-eng" };

export const content = {
  oneliner: `One tenant hogs shared resources.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>One tenant hogs shared resources. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>Noisy Neighbor</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
<p><b>Noisy Neighbor</b> affects how concurrent payment requests interact with Wallet, Order Service, Gateway, and Ledger under production load — not just in single-threaded dev environments.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Noisy Neighbor</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Noisy Neighbor</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Noisy Neighbor timeline">
<defs><marker id="fig-noisy-neighbor-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
</svg>`, caption: `How Noisy Neighbor unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Noisy Neighbor in request path">
<defs><marker id="fig-noisy-neighbor-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Noisy Neighbor</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-noisy-neighbor-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Noisy Neighbor</text>
</svg>`, caption: `Noisy Neighbor on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return mountSimulation(stage, panel, stageEl, {
    note: "Shared resource split across 4 tenants.",
    toggles: [{ key: "fix", label: "Per-tenant quotas (fair share)", kind: "ok", value: false }],
    frame(ctx) {
      const d = ctx.d; const fix = ctx.toggles.fix;
      // shares of 100%
      const shares = fix ? [25, 25, 25, 25] : [70, 12, 10, 8];
      const labels = ["Tenant A (batch)", "Tenant B", "Tenant C", "Tenant D"];
      shares.forEach((s, i) => {
        const y = 110 + i * 90;
        const starved = !fix && i > 0;
        d.text(120, y - 14, labels[i], { size: 13, color: i === 0 ? (fix ? C.ink : C.err) : (starved ? C.err : C.ink) });
        d.gauge(120, y, 500, 20, s / 100, { color: i === 0 ? (fix ? C.ledger : C.err) : (starved ? C.err : C.ok), value: s + "%" });
      });
      d.badge(500, 490, fix ? "quotas → fair 25% each" : "Tenant A hogs 70% — others starved", { color: fix ? C.ok : C.err, align: "center" });
      ctx.setStatus(fix ? "fair sharing enforced" : "noisy neighbor starving others", fix ? "ok" : "err");
    },
  });
}
