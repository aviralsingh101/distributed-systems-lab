...
export const meta = { id: "priority-queue-starvation", title: "Priority Queue Starvation", category: "prod-eng" };

export const content = {
  oneliner: `Low-priority jobs never run.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>Low-priority jobs never run. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>Priority Queue Starvation</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Priority Queue Starvation</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Priority Queue Starvation</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Priority Queue Starvation timeline">
<defs><marker id="fig-priority-queue-starvation-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
</svg>`, caption: `How Priority Queue Starvation unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Priority Queue Starvation in request path">
<defs><marker id="fig-priority-queue-starvation-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Priority Queu…</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-priority-queue-starvation-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Priority Queue Starvation</text>
</svg>`, caption: `Priority Queue Starvation on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return mountSimulation(stage, panel, stageEl, {
    note: "High-priority work arrives continuously.",
    toggles: [{ key: "fix", label: "Reserve 20% for low priority", kind: "ok", value: false }],
    frame(ctx, t) {
      const d = ctx.d; const fix = ctx.toggles.fix;
      // throughput split
      const hi = fix ? 80 : 100, lo = fix ? 20 : 0;
      d.text(300, 120, "High priority (real-time)", { size: 13, color: C.ink });
      d.gauge(300, 150, 400, 22, hi / 100, { color: C.gateway, value: hi + "%" });
      d.text(300, 240, "Low priority (batch refunds)", { size: 13, color: lo === 0 ? C.err : C.ink });
      d.gauge(300, 270, 400, 22, lo / 100, { color: lo === 0 ? C.err : C.ledger, value: lo + "%" });
      // waiting age of a low-prio job
      if (!fix) d.text(500, 360, "batch job age: " + (t % 120).toFixed(0) + "s and never scheduled", { size: 13, align: "center", color: C.err });
      else d.text(500, 360, "reserved lane keeps batch jobs moving", { size: 13, align: "center", color: C.ok });
      d.badge(500, 430, fix ? "both classes make progress" : "low-priority starved (0% throughput)", { color: fix ? C.ok : C.err, align: "center" });
      ctx.setStatus(fix ? "fair share via reserved capacity" : "strict priority starves low tier", fix ? "ok" : "err");
    },
  });
}
