...
<p>Interview tip: whiteboard the charge flow, mark where <b>Connection Pool Exhaustion</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ...
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Connection Pool Exhaustion timeline">
<defs><marker id="fig-connection-pool-exhaustion-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
</svg>`, caption: `How Connection Pool Exhaustion unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Connection Pool Exhaustion in request path">
<defs><marker id="fig-connection-pool-exhaustion-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Connection Po…</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-connection-pool-exhaustion-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Connection Pool Exhaustion</text>
</svg>`, caption: `Connection Pool Exhaustion on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  ...
  return mountSimulation(stage, panel, stageEl, {
    note: "Fixed pool of 10 DB connections.",
    params: [{ key: "hold", label: "Connection hold time", min: 1, max: 10, step: 1, value: 8, unit: "×", live: true }],
    frame(ctx, t) {
      const d = ctx.d; const hold = ctx.params.hold;
      const busy = Math.min(POOL, Math.round((hold / 10) * POOL + 1));
      const exhausted = busy >= POOL;
      // connection slots
      for (let i = 0; i < POOL; i++) {
        const x = 160 + (i % 5) * 140, y = 130 + Math.floor(i / 5) * 90;
        const inUse = i < busy;
        d.node(x - 55, y, 110, 56, { title: "conn " + i, color: inUse ? (exhausted ? C.err : C.warn) : C.ledger, state: inUse ? (exhausted ? "err" : "warn") : "ok", value: inUse ? "busy" : "free" });
      }
      const waiting = exhausted ? Math.round(hold * 2) : 0;
      d.gauge(300, 380, 400, 18, busy / POOL, { color: exhausted ? C.err : C.ok, label: "connections in use", value: busy + " / " + POOL });
      if (exhausted) d.badge(500, 440, waiting + " requests blocked waiting for a connection", { color: C.err, align: "center" });
      ctx.setStatus(exhausted ? "pool exhausted — requests blocking/timing out" : "connections available", exhausted ? "err" : "ok");
    },
  });
}
