...
<p>Interview tip: whiteboard the charge flow, mark where <b>Cascading Failure</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ...
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Cascading Failure timeline">
<defs><marker id="fig-cascading-failure-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
</svg>`, caption: `How Cascading Failure unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Cascading Failure in request path">
<defs><marker id="fig-cascading-failure-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Cascading Fai…</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-cascading-failure-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Cascading Failure</text>
</svg>`, caption: `Cascading Failure on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};

export function createSimulation(stage, panel, stageEl) {
  const chain = ["Client", "Order", "Payment", "Ledger"];
  return mountSimulation(stage, panel, stageEl, {
    note: "Ledger slows; does the failure climb the chain?",
    toggles: [{ key: "fix", label: "Timeouts + circuit breakers", kind: "ok", value: false }],
    frame(ctx, t) {
      const d = ctx.d; const fix = ctx.toggles.fix;
      const ph = phaseOf(t, [1.2, 1.2, 1.2, 1.6]);
      // failure originates at Ledger (index 3) and climbs
      chain.forEach((name, i) => {
        const y = 90 + i * 105;
        const depth = chain.length - 1 - i; // Ledger=0 climbs
        const affected = ph.i >= depth; // as phase grows, more climb
        let state = "ok", val = "healthy";
        if (i === chain.length - 1) { state = "err"; val = "slow (root cause)"; }
        else if (!fix && affected) { state = "err"; val = "threads exhausted"; }
        else if (fix && affected) { state = "warn"; val = "breaker open, fail fast"; }
        d.node(500 - 90, y, 180, 62, { title: name, color: i === 0 ? C.client : C.service, state, active: true, value: val });
        if (i < chain.length - 1) d.arrow(500, y + 62, 500, y + 105, { color: (!fix && affected) ? C.err : C.faint, width: 2, dashed: fix && affected });
      });
      const survived = fix;
      d.badge(500, 545, survived ? "contained at Payment — edge stays up" : "cascade reached the edge — total outage", { color: survived ? C.ok : C.err, align: "center" });
      ctx.setStatus(survived ? "failure contained by breakers" : "cascading failure climbing the chain", survived ? "ok" : "err");
    },
  });
}
