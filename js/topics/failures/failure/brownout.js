...
export const meta = { id: "brownout", title: "Brownout", category: "failure" };

export const content = {
  oneliner: `Dim optional work under load.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>Dim optional work under load. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>Brownout</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
<p><b>Brownout</b> affects how concurrent payment requests interact with Wallet, Order Service, Gateway, and Ledger under production load — not just in single-threaded dev environments.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Brownout</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Brownout</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Brownout timeline">
<defs><marker id="fig-brownout-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
</svg>`, caption: `How Brownout unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Brownout in request path">
<defs><marker id="fig-brownout-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Brownout</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-brownout-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Brownout</text>
</svg>`, caption: `Brownout on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return mountSimulation(stage, panel, stageEl, {
    note: "Optional work dims as load rises; core stays full.",
    params: [{ key: "load", label: "System load", min: 0, max: 100, step: 5, value: 40, unit: "%", live: true }],
    frame(ctx) {
      const d = ctx.d; const load = ctx.params.load;
      // dimming starts at 40% load, full off by 90%
      const dim = clamp((load - 40) / 50);
      const features = [
        { label: "Take payment (core)", quality: 1, core: true },
        { label: "Recommendations", quality: clamp(1 - dim * 1.2) },
        { label: "Rich animations", quality: clamp(1 - dim * 1.5) },
        { label: "Analytics / logging", quality: clamp(1 - dim) },
      ];
      features.forEach((f, i) => {
        const y = 110 + i * 84;
        const q = f.core ? 1 : f.quality;
        const col = f.core ? C.ledger : (q > 0.6 ? C.ok : q > 0.2 ? C.warn : C.err);
        d.text(300, y - 16, f.label, { size: 13, color: C.ink });
        d.gauge(300, y, 400, 16, q, { color: col, value: Math.round(q * 100) + "%" });
      });
      d.gauge(300, 500, 400, 14, load / 100, { color: load > 80 ? C.err : C.warn, label: "load", value: load + "%" });
      const survived = features[0].quality === 1;
      ctx.setStatus(load < 40 ? "full quality" : "brownout: optional work dimmed, core protected", "ok");
    },
  });
}
