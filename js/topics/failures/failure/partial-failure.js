...
export const meta = { id: "partial-failure", title: "Partial Failure", category: "failure" };

export const content = {
  oneliner: `Half the system works.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>Half the system works. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>Partial Failure</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
<p><b>Partial Failure</b> affects how concurrent payment requests interact with Wallet, Order Service, Gateway, and Ledger under production load — not just in single-threaded dev environments.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Partial Failure</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Partial Failure</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Partial Failure timeline">
<defs><marker id="fig-partial-failure-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
</svg>`, caption: `How Partial Failure unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Partial Failure in request path">
<defs><marker id="fig-partial-failure-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Partial Failu…</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-partial-failure-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Partial Failure</text>
</svg>`, caption: `Partial Failure on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return mountSimulation(stage, panel, stageEl, {
    note: "One dependency is down; requests fare differently.",
    toggles: [{ key: "fix", label: "Timeout + fallback for Fraud", kind: "ok", value: false }],
    frame(ctx, t) {
      const d = ctx.d; const fix = ctx.toggles.fix;
      const api = { x: 500, y: 90 };
      const svcs = [
        { x: 220, y: 300, label: "Wallet", up: true, c: C.ledger },
        { x: 500, y: 300, label: "Fraud", up: false, c: C.gateway },
        { x: 780, y: 300, label: "Ledger", up: true, c: C.service },
      ];
      d.node(api.x - 80, api.y - 26, 160, 52, { title: "Payment API", color: C.accent, active: true });
      svcs.forEach((s) => {
        d.node(s.x - 70, s.y - 26, 140, 52, { title: s.label, color: s.c, state: s.up ? "ok" : "err", active: true, value: s.up ? "up" : "DOWN" });
        const p = cycle(t + s.x / 300, 1);
        const reachable = s.up || fix; // with fix, fraud call short-circuits (fallback)
        const pos = d.along(api.x, api.y + 26, s.x, s.y - 26, p);
        d.arrow(api.x, api.y + 26, s.x, s.y - 26, { color: s.up ? C.faint : (fix ? C.warn : C.err), alpha: 0.3, head: false });
        d.token(pos.x, pos.y, { r: 7, color: s.up ? C.ok : (fix ? C.warn : C.err), glow: false });
      });
      d.badge(500, 470, fix ? "Fraud down → fallback (approve+flag); payments succeed" : "requests needing Fraud hang / fail", { color: fix ? C.ok : C.err, align: "center" });
      ctx.setStatus(fix ? "degraded but serving (timeout+fallback)" : "partial outage — some requests hang", fix ? "ok" : "err");
    },
  });
}
