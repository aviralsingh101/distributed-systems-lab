...
export const meta = { id: "fail-fast", title: "Fail Fast", category: "failure" };

export const content = {
  oneliner: `Reject immediately when overloaded.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>Reject immediately when overloaded. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>Fail Fast</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
<p><b>Fail Fast</b> affects how concurrent payment requests interact with Wallet, Order Service, Gateway, and Ledger under production load — not just in single-threaded dev environments.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Fail Fast</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Fail Fast</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Fail Fast timeline">
<defs><marker id="fig-fail-fast-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
</svg>`, caption: `How Fail Fast unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Fail Fast in request path">
<defs><marker id="fig-fail-fast-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Fail Fast</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-fail-fast-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Fail Fast</text>
</svg>`, caption: `Fail Fast on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return sequenceSim(stage, panel, stageEl, {
    note: "Overloaded gateway: wait for timeout, or reject now?",
    toggles: [{ key: "fix", label: "Fail fast (reject immediately)", kind: "ok", value: false }],
    scenario(ctx) {
      const fix = ctx.toggles.fix;
      const actors = [
        { id: "cl", label: "Client", color: C.client },
        { id: "g", label: "Gateway", color: C.gateway, kind: "db", value: "overloaded" },
        { id: "d", label: "Downstream", color: C.ledger, kind: "db", value: "saturated" },
      ];
      let steps;
      if (!fix) {
        steps = [
          { from: "cl", to: "g", label: "POST /pay", set: { cl: "waiting…" } },
          { from: "g", to: "d", label: "call (queued)", dashed: true, set: { g: "thread blocked" } },
          { from: "g", to: "g", label: "…30s timeout…", self: true, bad: true, set: { g: "still blocked" } },
          { from: "g", to: "cl", label: "504 after 30s", bad: true, set: { cl: "slow error" } },
        ];
      } else {
        steps = [
          { from: "cl", to: "g", label: "POST /pay", set: { cl: "waiting" } },
          { from: "g", to: "g", label: "queue full → reject", self: true, good: true, set: { g: "fast reject" } },
          { from: "g", to: "cl", label: "503 in 1ms", good: true, set: { cl: "fast error → fallback" } },
        ];
      }
      return {
        actors, steps, stepDur: 1.1,
        status: (r) => !r.done ? { text: "handling request…", cls: "" } : { text: fix ? "rejected in 1ms — thread freed" : "hung 30s then errored — thread wasted", cls: fix ? "ok" : "err" },
      };
    },
  });
}
