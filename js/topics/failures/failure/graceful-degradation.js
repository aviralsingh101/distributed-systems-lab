...
export const meta = { id: "graceful-degradation", title: "Graceful Degradation", category: "failure" };

export const content = {
  oneliner: `Drop non-essential features.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>Drop non-essential features. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>Graceful Degradation</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
<p><b>Graceful Degradation</b> affects how concurrent payment requests interact with Wallet, Order Service, Gateway, and Ledger under production load — not just in single-threaded dev environments.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Graceful Degradation</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Graceful Degradation</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Graceful Degradation timeline">
<defs><marker id="fig-graceful-degradation-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
</svg>`, caption: `How Graceful Degradation unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Graceful Degradation in request path">
<defs><marker id="fig-graceful-degradation-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Graceful Degr…</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-graceful-degradation-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Graceful Degradation</text>
</svg>`, caption: `Graceful Degradation on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return mountSimulation(stage, panel, stageEl, {
    note: "Some dependencies are down. What does the user get?",
    toggles: [{ key: "fix", label: "Degrade gracefully (drop optional)", kind: "ok", value: false }],
    frame(ctx, t) {
      const d = ctx.d; const fix = ctx.toggles.fix;
      const features = [
        { label: "Take payment", core: true, dep: "Wallet", up: true },
        { label: "Recommendations", core: false, dep: "Recs", up: false },
        { label: "Loyalty points", core: false, dep: "Rewards", up: false },
        { label: "Order confirmation", core: true, dep: "Ledger", up: true },
      ];
      features.forEach((f, i) => {
        const y = 110 + i * 90;
        let state, val;
        if (f.up) { state = "ok"; val = "working"; }
        else if (fix) { state = "warn"; val = "fallback / hidden"; }
        else { state = "err"; val = "ERROR (page broken)"; }
        d.node(300, y, 400, 60, { title: f.label + (f.core ? "  ·  core" : "  ·  optional"), color: f.core ? C.ledger : C.gateway, state, active: f.core, value: val });
      });
      const coreOk = fix || features.filter((f) => f.core).every((f) => f.up);
      const wholePageBroken = !fix && features.some((f) => !f.up);
      d.badge(500, 500, fix ? "core checkout works; optional features degraded" : "optional failure breaks the whole page", { color: fix ? C.ok : C.err, align: "center" });
      ctx.setStatus(fix ? "core path protected" : "no isolation — optional outage blocks core", fix ? "ok" : "err");
    },
  });
}
