...
export const meta = { id: "read-skew", title: "Read Skew", category: "concurrency" };

export const content = {
  oneliner: `Two rows read across an update; snapshot inconsistent.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>Two rows read across an update; snapshot inconsistent. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>Read Skew</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
<p><b>Read Skew</b> affects how concurrent payment requests interact with Wallet, Order Service, Gateway, and Ledger under production load — not just in single-threaded dev environments.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Read Skew</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Read Skew</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Read Skew timeline">
<defs><marker id="fig-read-skew-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
</svg>`, caption: `How Read Skew unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Read Skew in request path">
<defs><marker id="fig-read-skew-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Read Skew</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-read-skew-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Read Skew</text>
</svg>`, caption: `Read Skew on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return sequenceSim(stage, panel, stageEl, {
    note: "Reader samples A then B; a transfer commits between.",
    params: [{ key: "total", label: "A + B invariant", min: 100, max: 400, step: 20, value: 200 }],
    toggles: [{ key: "fix", label: "Read from one snapshot", kind: "ok", value: false }],
    scenario(ctx) {
      const total = ctx.params.total, half = total / 2, fix = ctx.toggles.fix;
      const actors = [
        { id: "a", label: "Wallet A", color: C.wallet, kind: "db", value: String(half) },
        { id: "r", label: "Reconciler", color: C.service },
        { id: "b", label: "Wallet B", color: C.wallet, kind: "db", value: String(half) },
      ];
      const readB = fix ? half : 0;
      const steps = [
        { from: "r", to: "a", label: "read A = " + half, set: { r: "A=" + half } },
        { from: "b", to: "a", label: "transfer " + half + " B→A", set: { a: String(total), b: "0" } },
        { from: "r", to: "b", label: "read B = " + readB, good: fix, bad: !fix, set: { r: `A+B=${half + readB}` } },
      ];
      return {
        actors, steps, stepDur: 1.2,
        status: (r) => !r.done ? { text: "sampling A then B…", cls: "" }
          : fix ? { text: `snapshot sum = ${total} ✓`, cls: "ok" } : { text: `observed sum = ${half} ≠ ${total}`, cls: "err" },
      };
    },
  });
}
