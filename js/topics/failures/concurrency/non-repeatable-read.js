...
export const meta = { id: "non-repeatable-read", title: "Non-repeatable Read", category: "concurrency" };

export const content = {
  oneliner: `Same row, two reads, two values.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>Same row, two reads, two values. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>Non-repeatable Read</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
<p><b>Non-repeatable Read</b> affects how concurrent payment requests interact with Wallet, Order Service, Gateway, and Ledger under production load — not just in single-threaded dev environments.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Non-repeatable Read</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Non-repeatable Read</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Non-repeatable Read timeline">
<defs><marker id="fig-non-repeatable-read-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
</svg>`, caption: `How Non-repeatable Read unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Non-repeatable Read in request path">
<defs><marker id="fig-non-repeatable-read-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Non-repeatabl…</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-non-repeatable-read-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Non-repeatable Read</text>
</svg>`, caption: `Non-repeatable Read on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return sequenceSim(stage, panel, stageEl, {
    note: "T1 reads a row twice; T2 commits a change between.",
    params: [{ key: "v0", label: "Initial", min: 0, max: 200, step: 10, value: 100 },
             { key: "v1", label: "T2 writes", min: 0, max: 200, step: 10, value: 40 }],
    toggles: [{ key: "fix", label: "Repeatable Read snapshot", kind: "ok", value: false }],
    scenario(ctx) {
      const { v0, v1 } = ctx.params; const fix = ctx.toggles.fix;
      const actors = [
        { id: "t1", label: "T1 (reader)", color: C.service },
        { id: "db", label: "Wallet", color: C.ledger, kind: "db", value: String(v0) },
        { id: "t2", label: "T2 (writer)", color: C.gateway },
      ];
      const steps = [
        { from: "t1", to: "db", label: "read1 = " + v0, set: { t1: "got " + v0 } },
        { from: "t2", to: "db", label: "UPDATE→" + v1 + " commit", set: { db: String(v1) } },
        { from: "t1", to: "db", label: "read2 = " + (fix ? v0 : v1), good: fix, bad: !fix, set: { t1: "got " + (fix ? v0 : v1) } },
      ];
      return {
        actors, steps, stepDur: 1.2,
        status: (r) => !r.done ? { text: "reading twice…", cls: "" }
          : fix ? { text: `both reads = ${v0} (snapshot)`, cls: "ok" } : { text: `read ${v0} then ${v1}`, cls: "err" },
      };
    },
  });
}
