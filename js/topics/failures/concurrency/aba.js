...
export const meta = { id: "aba", title: "ABA Problem", category: "concurrency" };

export const content = {
  oneliner: `Value changed A→B→A; CAS is fooled.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>Value changed A→B→A; CAS is fooled. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>ABA Problem</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
<p><b>ABA Problem</b> affects how concurrent payment requests interact with Wallet, Order Service, Gateway, and Ledger under production load — not just in single-threaded dev environments.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>ABA Problem</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>ABA Problem</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ABA Problem timeline">
<defs><marker id="fig-aba-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
</svg>`, caption: `How ABA Problem unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="ABA Problem in request path">
<defs><marker id="fig-aba-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">ABA Problem</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-aba-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — ABA Problem</text>
</svg>`, caption: `ABA Problem on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return sequenceSim(stage, panel, stageEl, {
    note: "Slow reader A vs fast mutator B on one slot.",
    toggles: [{ key: "fix", label: "Tag the value with a version", kind: "ok", value: false }],
    scenario(ctx) {
      const fix = ctx.toggles.fix;
      const actors = [
        { id: "a", label: "Reader A", color: C.service },
        { id: "slot", label: "Slot", color: C.ledger, kind: "db", value: fix ? "A · v0" : "A" },
        { id: "b", label: "Mutator B", color: C.gateway },
      ];
      const steps = [
        { from: "a", to: "slot", label: fix ? "read (A,v0)" : "read A", set: { a: fix ? "A,v0" : "A" } },
        { from: "b", to: "slot", label: "A → B", set: { slot: fix ? "B · v1" : "B" } },
        { from: "b", to: "slot", label: "B → A (reuse)", set: { slot: fix ? "A · v2" : "A" } },
        fix
          ? { from: "a", to: "slot", label: "CAS(A,v0) — v is v2 ✕", bad: true, dashed: true, set: { a: "retry" } }
          : { from: "a", to: "slot", label: "CAS(A→X) ✓ (fooled)", bad: true, set: { a: "wrong!" } },
      ];
      return {
        actors, steps, stepDur: 1.15,
        status: (r) => !r.done ? { text: "A→B→A in flight…", cls: "" }
          : fix ? { text: "stale CAS rejected (v0 ≠ v2)", cls: "ok" }
                : { text: "CAS fooled — value looked unchanged", cls: "err" },
      };
    },
  });
}
