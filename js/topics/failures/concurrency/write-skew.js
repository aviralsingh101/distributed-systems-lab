...
export const meta = { id: "write-skew", title: "Write Skew", category: "concurrency" };

export const content = {
  oneliner: `Two valid writes jointly break a rule.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>Two valid writes jointly break a rule. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>Write Skew</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
<p><b>Write Skew</b> affects how concurrent payment requests interact with Wallet, Order Service, Gateway, and Ledger under production load — not just in single-threaded dev environments.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Write Skew</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Write Skew</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Write Skew timeline">
<defs><marker id="fig-write-skew-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
</svg>`, caption: `How Write Skew unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Write Skew in request path">
<defs><marker id="fig-write-skew-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Write Skew</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-write-skew-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Write Skew</text>
</svg>`, caption: `Write Skew on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return sequenceSim(stage, panel, stageEl, {
    note: "Two cards, one shared spending limit.",
    params: [
      { key: "limit", label: "Shared limit", min: 60, max: 200, step: 10, value: 100 },
      { key: "each", label: "Each purchase", min: 40, max: 120, step: 10, value: 80 },
    ],
    toggles: [{ key: "fix", label: "Serializable / lock shared row", kind: "ok", value: false }],
    scenario(ctx) {
      const { limit, each } = ctx.params;
      const actors = [
        { id: "t1", label: "T1 · Card A", color: C.service },
        { id: "lim", label: "limit row", color: C.ledger, kind: "db", value: "0/" + limit },
        { id: "t2", label: "T2 · Card B", color: C.gateway },
      ];
      let steps;
      if (!ctx.toggles.fix) {
        steps = [
          { from: "t1", to: "lim", label: `read 0≤${limit} ✓`, good: true, set: { t1: "ok" } },
          { from: "t2", to: "lim", label: `read 0≤${limit} ✓`, good: true, set: { t2: "ok" } },
          { from: "t1", to: "lim", label: "charge " + each, set: { lim: each + "/" + limit } },
          { from: "t2", to: "lim", label: "charge " + each, bad: true, set: { lim: 2 * each + "/" + limit, t2: "over limit!" } },
        ];
      } else {
        steps = [
          { from: "t1", to: "lim", label: "lock + charge ✓", good: true, set: { lim: each + "/" + limit, t1: "committed" } },
          { from: "t2", to: "lim", label: "conflict → abort", bad: true, dashed: true, set: { t2: "aborted, retry" } },
        ];
      }
      return {
        actors, steps, stepDur: 1.15,
        status: (r) => !r.done ? { text: "both read the shared limit…", cls: "" }
          : ctx.toggles.fix ? { text: `total ${each}/${limit} (capped)`, cls: "ok" }
                            : { text: `total ${2 * each}/${limit} — invariant broken`, cls: "err" },
      };
    },
  });
}
