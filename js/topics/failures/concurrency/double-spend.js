...
export const meta = { id: "double-spend", title: "Double Spend", category: "concurrency" };

export const content = {
  oneliner: `Same balance spent twice.`,
  archetype: "failure",
  sections: [
    { title: `Symptom`, body: `<p>Same balance spent twice. In production this surfaces as customer-visible errors, reconciliation drift, or SLO burn without an obvious code exception — support tickets reference wrong balances, duplicate charges, or timeouts during peak checkout.</p>
<p>Metrics to watch: error rate spike on affected endpoints, p99 latency increase, consumer lag, or reconciliation job failures comparing Ledger totals to Wallet projections.</p>` },
    { title: `Root cause`, body: `<p><b>Double Spend</b> occurs when the system assumes single-threaded or always-success execution. Under parallel charges, retries, or partial outages, that assumption breaks.</p>
<p><b>Double Spend</b> affects how concurrent payment requests interact with Wallet, Order Service, Gateway, and Ledger under production load — not just in single-threaded dev environments.</p>
...
<p>Document the chosen fix in the service runbook and add an integration test that fails without it.</p>` },
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Double Spend</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Double Spend</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Double Spend timeline">
<defs><marker id="fig-double-spend-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
</svg>`, caption: `How Double Spend unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Double Spend in request path">
<defs><marker id="fig-double-spend-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Double Spend</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-double-spend-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Double Spend</text>
</svg>`, caption: `Double Spend on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return sequenceSim(stage, panel, stageEl, {
    note: "Same wallet, two simultaneous full-balance spends.",
    params: [{ key: "bal", label: "Balance", min: 50, max: 200, step: 10, value: 100 }],
    toggles: [{ key: "fix", label: "Atomic guarded UPDATE / serializable", kind: "ok", value: false }],
    scenario(ctx) {
      const bal = ctx.params.bal;
      const actors = [
        { id: "d1", label: "Device 1", color: C.service },
        { id: "db", label: "Ledger", color: C.ledger, kind: "db", value: String(bal) },
        { id: "d2", label: "Device 2", color: C.gateway },
      ];
      let steps;
      if (!ctx.toggles.fix) {
        steps = [
          { from: "d1", to: "db", label: "read " + bal, set: { d1: "sees " + bal } },
          { from: "d2", to: "db", label: "read " + bal, set: { d2: "sees " + bal } },
          { from: "d1", to: "db", label: "spend " + bal, good: true, set: { db: "0" } },
          { from: "d2", to: "db", label: "spend " + bal, bad: true, set: { db: String(-bal), d2: "double spent!" } },
        ];
      } else {
        steps = [
          { from: "d1", to: "db", label: "UPDATE…≥ ✓", good: true, set: { db: "0", d1: "1 row" } },
          { from: "d2", to: "db", label: "0≥" + bal + "? no", bad: true, dashed: true, set: { d2: "0 rows — declined" } },
        ];
      }
      return {
        actors, steps, stepDur: 1.15,
        status: (r) => !r.done ? { text: "both reading…", cls: "" }
          : ctx.toggles.fix ? { text: "balance 0 — spent once (safe)", cls: "ok" }
                            : { text: `balance ${-bal} — spent twice`, cls: "err" },
      };
    },
  });
}
