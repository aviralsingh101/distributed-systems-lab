...
<p>Interview tip: whiteboard the charge flow, mark where <b>Dogpile Effect</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ...
  figures: [
    { id: "timeline", svg: `<svg viewBox="0 0 520 200" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Dogpile Effect timeline">
...
<line x1="90" y1="56" x2="118" y2="80" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
<line x1="90" y1="106" x2="118" y2="88" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
<line x1="210" y1="80" x2="238" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
<line x1="210" y1="88" x2="238" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
<line x1="320" y1="56" x2="338" y2="56" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
<line x1="320" y1="106" x2="338" y2="106" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
<line x1="420" y1="56" x2="438" y2="78" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
<line x1="420" y1="106" x2="438" y2="92" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
</svg>`, caption: `How Dogpile Effect unfolds — two workers interleave on the same resource; the second write can overwrite the first.` },
    { id: "request-path", svg: `<svg viewBox="0 0 640 120" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Dogpile Effect in request path">
<defs><marker id="fig-dogpile-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#5b9dff"/></marker></defs>
...
<text x="144" y="52" text-anchor="middle" fill="#cdd6e8" font-size="11" font-family="system-ui">Dogpile Effect</text><text x="144" y="72" text-anchor="middle" fill="#93a1bd" font-size="9" font-family="system-ui">this topic</text>
...
<line x1="82" y1="58" x2="98" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
<line x1="188" y1="58" x2="204" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
<line x1="286" y1="58" x2="302" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
<line x1="388" y1="58" x2="404" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
<line x1="478" y1="58" x2="494" y2="58" stroke="#5b9dff" stroke-width="1.5" marker-end="url(#fig-dogpile-arr)"/>
<text x="320" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">HTTPS request flow — Dogpile Effect</text>
</svg>`, caption: `Dogpile Effect on the payment request path — from client charge to Ledger commit.` }
  ],
  related: [],
};
...
export function createSimulation(stage, panel, stageEl) {
  return mountSimulation(stage, panel, stageEl, {
    note: "Several workers rebuild the same value.",
    toggles: [{ key: "fix", label: "Shared rebuild lock", kind: "ok", value: false }],
    frame(ctx, t) {
      const d = ctx.d; const fix = ctx.toggles.fix;
      const backend = { x: 760, y: 250 };
      const ph = phaseOf(t, [1.2, 2.0]);
      d.node(backend.x - 90, backend.y - 45, 180, 90, { title: "Backend", color: C.ledger, state: (!fix && ph.i === 1) ? "err" : "ok", active: true, value: "load ×" + (fix ? 1 : 4) });
      const workers = 4; let hits = 0;
      for (let i = 0; i < workers; i++) {
        const wy = 110 + i * 90;
        d.node(90, wy, 130, 52, { title: "worker " + (i + 1), color: C.service });
        if (ph.i === 1) {
          if (!fix) { const pos = d.along(220, wy + 26, backend.x - 90, backend.y, ph.p); d.token(pos.x, pos.y, { r: 7, color: C.err }); hits++; }
          else if (i === 0) { const pos = d.along(220, wy + 26, backend.x - 90, backend.y, ph.p); d.token(pos.x, pos.y, { r: 8, color: C.ok }); hits = 1; d.lock(300, wy + 26, { color: C.ok }); }
          else d.text(250, wy + 26, "waits (locked)", { size: 11, color: C.muted });
        }
      }
      ctx.setStatus(fix ? "1 worker rebuilds (lock held)" : (ph.i === 1 ? workers + " workers rebuild the same value" : "detecting miss"), fix ? "ok" : (ph.i === 1 ? "err" : ""));
    },
  });
}
