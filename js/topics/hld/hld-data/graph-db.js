...
<p>Distinguish <b>Graph DB</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Traverse relationships natively.</p><p>Document sync vs async usage: blocking calls belong on the critical user path only when necessary; everything else should be queued or event-driven with clear compensation.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p><p>Clarify schema flexibility, query patterns, compaction, and how analytics workloads are isolated from OLTP. Name the partition or sort key that drives locality.</p>`, figureAfter: "architecture" },
    { title: `HLD placement`, body: `<p><b>Graph DB</b> belongs in the persistence/analytics layer. Show read vs write paths, replication, and which queries hit which store.</p><p>Separate OLTP hot path from batch/analytics; mark ETL or CDC if data is copied.</p>` },
...
<p><b>Failure isolation:</b> If <b>Graph DB</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
