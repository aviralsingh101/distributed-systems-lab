...
export const content = {
  oneliner: `Async index is stale.`,
  archetype: "concept",
  sections: [
    { title: `What is Secondary Index Lag?`, body: `<p><b>Secondary Index Lag</b> — Async index is stale.</p>
...
<p>Unlike a generic "best practice" label, it has a specific seat in the payment platform architecture with defined inputs, outputs, and failure modes.</p>` },
    { title: `How it works`, body: `<p>At runtime, components interact in a defined order with configurable timeouts, health checks, and backoff policies. Trace a single <code>POST /v1/charge</code> with <code>X-Request-ID</code> to see where <b>Secondary Index Lag</b> applies.</p>
...
<p>Capacity planning: measure peak QPS, payload size, and fan-out before scaling horizontally. Tail latency (p99) often reveals misconfiguration before mean latency moves.</p>` },
    { title: `In production`, body: `<p>Operate with dashboards: error rate, p99 latency, saturation, and dependency health. Runbooks cover failover, key rotation, and rollback. Game-day exercises validate that <b>Secondary Index Lag</b> behaves correctly during AZ failure or broker restart.</p>
<p>Security: TLS on public paths; no PAN in application logs; audit admin APIs that change <b>Secondary Index Lag</b> configuration.</p>` },
...
<li>Operating without correlation IDs across Order → Gateway → Ledger.</li>
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Secondary Index Lag</b> changes to production:</p>
<ul>
