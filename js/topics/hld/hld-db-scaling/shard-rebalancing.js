...
<p>Distinguish <b>Shard Rebalancing</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Moving data disrupts traffic.</p><p>Trace one end-to-end request through <b>Shard Rebalancing</b>: what triggers it, which components participate, where state is stored, and what the client observes on success vs timeout.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p>`, figureAfter: "request-flow" },
    { title: `HLD placement`, body: `<p><b>Shard Rebalancing</b> appears at the shard/replica layer. Show partition key, hot spots, and cross-shard paths.</p>` },
...
<p><b>Failure isolation:</b> If <b>Shard Rebalancing</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
...
export function createSimulation(stage, panel, stageEl) {
  return createTopicSim("shard-rebalancing", stage, panel, stageEl);
}