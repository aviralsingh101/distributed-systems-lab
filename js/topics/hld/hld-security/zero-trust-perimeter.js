...
<p>Distinguish <b>Zero-Trust Perimeter</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Never trust, always verify.</p><p>Define SLIs affected by <b>Zero-Trust Perimeter</b> — latency, availability, error rate — and which dependency failure degrades each.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p><p>Map trust zones: public internet, DMZ, service mesh, data store. Show where authentication ends and authorization begins on every API.</p>`, figureAfter: "request-flow" },
    { title: `HLD placement`, body: `<p><b>Zero-Trust Perimeter</b> wraps trust boundaries: perimeter, service-to-service, and data-at-rest. Draw where tokens are validated and secrets never cross.</p>` },
...
<p><b>Failure isolation:</b> If <b>Zero-Trust Perimeter</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
