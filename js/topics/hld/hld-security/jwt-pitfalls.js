...
<p>Distinguish <b>JWT Pitfalls</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Expiry, rotation, and token size traps.</p><p>Document sync vs async usage: blocking calls belong on the critical user path only when necessary; everything else should be queued or event-driven with clear compensation.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p><p>Map trust zones: public internet, DMZ, service mesh, data store. Show where authentication ends and authorization begins on every API.</p>`, figureAfter: "request-flow" },
    { title: `HLD placement`, body: `<p><b>JWT Pitfalls</b> wraps trust boundaries: perimeter, service-to-service, and data-at-rest. Draw where tokens are validated and secrets never cross.</p>` },
...
<p><b>Failure isolation:</b> If <b>JWT Pitfalls</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
