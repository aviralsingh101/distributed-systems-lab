...
<p>Interview tip: whiteboard the charge flow, mark where <b>Consistent Hashing (placement)</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ...
  template: "topology",
  sim: () => ({
    note: `Explore Consistent Hashing (placement) in the payment platform.`,
    toggles: [{ key: "fix", label: "Apply Consistent Hashing (placement)", kind: "ok", value: false }],
    nodes: (ctx) => [
    ...
    edges: (ctx) => [
      ...
      { from: "l", to: "q", active: ctx.toggles.fix, label: "Consistent Hashing (placement)" },
    ],
    activeEdge: (ctx, t) => ctx.toggles.fix ? { from: "l", to: "q" } : { from: "c", to: "o" },
    status: (ctx) => ({ text: ctx.toggles.fix ? "Consistent Hashing (placement) in path" : "pattern absent", cls: ctx.toggles.fix ? "ok" : "warn" }),
  }),
});
