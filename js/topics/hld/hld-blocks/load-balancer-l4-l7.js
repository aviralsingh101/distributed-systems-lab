...
<p>Distinguish <b>Load Balancer (L4 / L7)</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Distribute traffic by IP or HTTP semantics.</p><p>Trace one end-to-end request through <b>Load Balancer (L4 / L7)</b>: what triggers it, which components participate, where state is stored, and what the client observes on success vs timeout.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p>`, figureAfter: "request-flow" },
    { title: `HLD placement`, body: `<p><b>Load Balancer (L4 / L7)</b> is infrastructure glue: draw it between clients and application tier, or between services and data/queue tiers. Mark whether it is stateful, horizontally scaled, and what fails independently.</p><p>Clarify single point of failure vs HA pair, and what config change requires drain vs hot reload.</p>` },
...
<p><b>Failure isolation:</b> If <b>Load Balancer (L4 / L7)</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
...
export function createSimulation(stage, panel, stageEl) {
  return createTopicSim("load-balancer-l4-l7", stage, panel, stageEl);
}