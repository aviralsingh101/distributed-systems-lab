...
<p>Interview tip: whiteboard the charge flow, mark where <b>SQL vs NoSQL Selection</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ...
  figures: [
    { id: "comparison", svg: `<svg viewBox="0 0 480 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="SQL vs NoSQL Selection comparison">
...
<text x="240" y="30" text-anchor="middle" fill="#cdd6e8" font-size="12" font-weight="600" font-family="system-ui">SQL vs NoSQL Selection</text>
...
<text x="240" y="135" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">Prototype under realistic load before choosing</text>
</svg>`, caption: `Decision fork for SQL vs NoSQL Selection — weigh simplicity vs scale before committing.` }
  ],
  related: [],
  ...
  template: "tradeoff",
  sim: () => ({
    note: `Explore SQL vs NoSQL Selection in the payment platform.`,
    toggleLabel: "Switch approach",
    labelA: "Without pattern",
    labelB: "With SQL vs NoSQL Selection",
    sideA: () => ({ nodes: [
    ...
    sideB: () => ({ nodes: [
      ...
      { title: "SQL vs NoSQL Selection", value: "applied" },
      { title: "Independent scale", value: "ok" },
    ]}),
    status: (ctx, t, useB) => ({ text: useB ? "SQL vs NoSQL Selection — better fit" : "naive — hits limits", cls: useB ? "ok" : "warn" }),
  }),
});
