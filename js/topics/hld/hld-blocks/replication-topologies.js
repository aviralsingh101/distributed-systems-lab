...
<p>Distinguish <b>Replication Topologies</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Primary-replica, multi-leader, leaderless.</p><p>Define SLIs affected by <b>Replication Topologies</b> — latency, availability, error rate — and which dependency failure degrades each.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p>`, figureAfter: "request-flow" },
    { title: `HLD placement`, body: `<p><b>Replication Topologies</b> is infrastructure glue: draw it between clients and application tier, or between services and data/queue tiers. Mark whether it is stateful, horizontally scaled, and what fails independently.</p><p>Clarify single point of failure vs HA pair, and what config change requires drain vs hot reload.</p>` },
...
<p><b>Failure isolation:</b> If <b>Replication Topologies</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
