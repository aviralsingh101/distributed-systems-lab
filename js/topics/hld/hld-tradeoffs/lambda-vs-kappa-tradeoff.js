...
const topic = makeTopic({
  ...
  tier: "essential",
  archetype: "tradeoff",
  oneliner: `Dual pipeline vs stream-only simplification.`,
  sections: [
    { title: `The decision`, body: `<p>Dual pipeline vs stream-only simplification. This is an architectural fork — not a universal winner. The right choice depends on consistency requirements, team expertise, operational budget, and how your payment platform scales.</p>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Lambda vs Kappa</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ...
  figures: [
    { id: "comparison", svg: `<svg viewBox="0 0 480 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Lambda vs Kappa comparison">
...
<text x="240" y="30" text-anchor="middle" fill="#cdd6e8" font-size="12" font-weight="600" font-family="system-ui">Lambda vs Kappa</text>
...
<text x="240" y="135" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">Prototype under realistic load before choosing</text>
</svg>`, caption: `Decision fork for Lambda vs Kappa — weigh simplicity vs scale before committing.` }
  ],
  related: [],
  ...
  template: "tradeoff",
  sim: () => ({
    note: `Explore Lambda vs Kappa in the payment platform.`,
    toggleLabel: "Switch approach",
    labelA: "Without pattern",
    labelB: "With Lambda vs Kappa",
    sideA: () => ({ nodes: [
    ...
    sideB: () => ({ nodes: [
      ...
      { title: "Lambda vs Kappa", value: "applied" },
      { title: "Independent scale", value: "ok" },
    ]}),
    status: (ctx, t, useB) => ({ text: useB ? "Lambda vs Kappa — better fit" : "naive — hits limits", cls: useB ? "ok" : "warn" }),
  }),
});
