...
<p>Distinguish <b>Poison Pill Handling</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Stop bad messages from blocking queues.</p><p>Document sync vs async usage: blocking calls belong on the critical user path only when necessary; everything else should be queued or event-driven with clear compensation.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p>`, figureAfter: "request-flow" },
    { title: `HLD placement`, body: `<p><b>Poison Pill Handling</b> spans the operational layer — health checks, deploy paths, observability hooks. Frame as design choices that affect architecture, not a post-launch runbook.</p>` },
...
<p><b>Failure isolation:</b> If <b>Poison Pill Handling</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
