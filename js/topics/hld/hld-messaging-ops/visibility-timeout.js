...
<p>Distinguish <b>Visibility Timeout</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Crash → message re-appears.</p><p>Define SLIs affected by <b>Visibility Timeout</b> — latency, availability, error rate — and which dependency failure degrades each.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p>`, figureAfter: "request-flow" },
    { title: `HLD placement`, body: `<p><b>Visibility Timeout</b> lives in the async pipeline between producers and consumers. Mark DLQ, visibility, and rebalance behavior.</p>` },
...
<p><b>Failure isolation:</b> If <b>Visibility Timeout</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
