...
const topic = makeTopic({
  ...
  tier: "essential",
  archetype: "tradeoff",
  oneliner: `Customer promise vs internal reliability target.`,
  sections: [
    { title: `The decision`, body: `<p>Customer promise vs internal reliability target. This is an architectural fork — not a universal winner. The right choice depends on consistency requirements, team expertise, operational budget, and how your payment platform scales.</p>
...
<p>Prototype both paths under realistic parallel charge load before committing — paper comparisons miss tail latency, retry storms, and reconciliation toil.</p>` },
    { title: `Decision guide for SLA vs SLO`, body: `<p>Choose the simpler option that meets current SLOs. Escalate complexity only when metrics prove failure: duplicate charges, unreconciled settlements, p99 breaches during peak, or ops toil blocking feature velocity.</p>
<p>Regardless of choice, instrument <b>SLA vs SLO</b> with metrics, run game-days, and keep rollback documented before any migration.</p>` },
    { title: `Production checklist`, body: `<p>Before committing to either side of <b>SLA vs SLO</b>: load-test peak checkout, measure reconciliation drift, document RTO/RPO, and ensure on-call runbooks cover the failure modes each option introduces.</p>` },
    { title: `Production checklist`, body: `<p>Before shipping <b>SLA vs SLO</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>SLA vs SLO</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
    { id: "comparison", svg: `<svg viewBox="0 0 480 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="SLA vs SLO comparison">
...
<text x="240" y="30" text-anchor="middle" fill="#cdd6e8" font-size="12" font-weight="600" font-family="system-ui">SLA vs SLO</text>
...
<text x="240" y="135" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">Prototype under realistic load before choosing</text>
</svg>`, caption: `Decision fork for SLA vs SLO — weigh simplicity vs scale before committing.` }
  ],
  related: [],
  ...
  template: "tradeoff",
  sim: () => ({
    note: `Explore SLA vs SLO in the payment platform.`,
    toggleLabel: "Switch approach",
    labelA: "Without pattern",
    labelB: "With SLA vs SLO",
    sideA: () => ({ nodes: [
    ...
    sideB: () => ({ nodes: [
      ...
      { title: "SLA vs SLO", value: "applied" },
      { title: "Independent scale", value: "ok" },
    ]}),
    status: (ctx, t, useB) => ({ text: useB ? "SLA vs SLO — better fit" : "naive — hits limits", cls: useB ? "ok" : "warn" }),
  }),
});
