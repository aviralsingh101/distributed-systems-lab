...
const topic = makeTopic({
  ...
  sections: [
    { title: `Functional requirements`, body: `<ul><li>Ingest time-series; downsampling; alerting.</li><li>Cardinality control; long-term storage.</li><li>Query API for dashboards.</li></ul>` },
    { title: `Capacity sketch`, body: `<p>Estimate DAU → peak QPS, storage/day, and fan-out factor. For <b>Metrics / Monitoring System</b>, identify the dominant cost: bandwidth, storage, or compute.</p>
...
<p>Call out read:write ratio; it drives cache vs write-optimized store choice.</p>` },
    { title: `High-level design`, body: `<p>Agents push/pull → ingestion with label validation → TSDB shard by time → downsampler → alert evaluator → notification router.</p>`, figureAfter: "architecture" },
    { title: `Deep dives`, body: `<p>Pick 2–3 areas interviewers probe: data model, partitioning, caching, consistency. For <b>Metrics / Monitoring System</b>, explain id generation, hot keys, and what fails under 10× load.</p>
