...
const topic = makeTopic({
  ...
  sections: [
    { title: `Functional requirements`, body: `<ul><li>Match riders/drivers; ETA; surge pricing.</li><li>Geospatial indexing; real-time location.</li><li>Strong consistency on trip state.</li></ul>` },
    { title: `Capacity sketch`, body: `<p>Estimate DAU → peak QPS, storage/day, and fan-out factor. For <b>Ride-Sharing Dispatch</b>, identify the dominant cost: bandwidth, storage, or compute.</p>
...
<p>Call out read:write ratio; it drives cache vs write-optimized store choice.</p>` },
    { title: `High-level design`, body: `<p>Geohash index for drivers; matching service scores candidates; trip state machine; location stream via Kafka; surge as dynamic pricing service.</p>`, figureAfter: "architecture" },
    { title: `Deep dives`, body: `<p>Pick 2–3 areas interviewers probe: data model, partitioning, caching, consistency. For <b>Ride-Sharing Dispatch</b>, explain id generation, hot keys, and what fails under 10× load.</p>
