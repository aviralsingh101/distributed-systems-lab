...
const topic = makeTopic({
  ...
  sections: [
    { title: `Functional requirements`, body: `<ul><li>Upload, transcode, adaptive bitrate streaming.</li><li>CDN distribution; resume playback.</li><li>Global latency &lt; 2s start time target.</li></ul>` },
    { title: `Capacity sketch`, body: `<p>Estimate DAU → peak QPS, storage/day, and fan-out factor. For <b>Video Streaming</b>, identify the dominant cost: bandwidth, storage, or compute.</p>
...
<p>Call out read:write ratio; it drives cache vs write-optimized store choice.</p>` },
    { title: `High-level design`, body: `<p>Upload → transcode workers → segment store → CDN edge → client adaptive player. Separate control plane (metadata) from data plane (bytes).</p>`, figureAfter: "architecture" },
    { title: `Deep dives`, body: `<p>Pick 2–3 areas interviewers probe: data model, partitioning, caching, consistency. For <b>Video Streaming</b>, explain id generation, hot keys, and what fails under 10× load.</p>
