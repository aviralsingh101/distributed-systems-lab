...
const topic = makeTopic({
  ...
  sections: [
    { title: `Functional requirements`, body: `<ul><li>Concurrent edits; low latency sync.</li><li>Conflict resolution (OT/CRDT).</li><li>Offline edits merge on reconnect.</li></ul>` },
    { title: `Capacity sketch`, body: `<p>Estimate DAU → peak QPS, storage/day, and fan-out factor. For <b>Collaborative Editor</b>, identify the dominant cost: bandwidth, storage, or compute.</p>
...
<p>Call out read:write ratio; it drives cache vs write-optimized store choice.</p>` },
    { title: `High-level design`, body: `<p>CRDT/OT document model; WebSocket sync; snapshot + op log in DB. Presence channel separate from document channel.</p>`, figureAfter: "architecture" },
    { title: `Deep dives`, body: `<p>Pick 2–3 areas interviewers probe: data model, partitioning, caching, consistency. For <b>Collaborative Editor</b>, explain id generation, hot keys, and what fails under 10× load.</p>
