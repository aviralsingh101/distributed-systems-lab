...
const topic = makeTopic({
  ...
  sections: [
    { title: `Functional requirements`, body: `<ul><li>Upload/download objects; metadata; versioning.</li><li>11 nines durability; multipart for large files.</li><li>Event notifications on object changes.</li></ul>` },
    { title: `Capacity sketch`, body: `<p>Estimate DAU → peak QPS, storage/day, and fan-out factor. For <b>File Storage (S3-like)</b>, identify the dominant cost: bandwidth, storage, or compute.</p>
...
<p>Call out read:write ratio; it drives cache vs write-optimized store choice.</p>` },
    { title: `High-level design`, body: `<p><b>Components:</b> metadata DB, blob store (object nodes), load balancer, background erasure coding/replication. Hash object key → partition. CDN for hot objects.</p>`, figureAfter: "architecture" },
    { title: `Deep dives`, body: `<p>Pick 2–3 areas interviewers probe: data model, partitioning, caching, consistency. For <b>File Storage (S3-like)</b>, explain id generation, hot keys, and what fails under 10× load.</p>
