...
const topic = makeTopic({
  ...
  sections: [
    { title: `Functional requirements`, body: `<ul><li>Multi-channel: push, email, SMS.</li><li>Template + preference management; rate limits per user.</li><li>At-least-once delivery; idempotent sends.</li></ul>` },
    { title: `Capacity sketch`, body: `<p>Estimate DAU → peak QPS, storage/day, and fan-out factor. For <b>Notification System</b>, identify the dominant cost: bandwidth, storage, or compute.</p>
...
<p>Call out read:write ratio; it drives cache vs write-optimized store choice.</p>` },
    { title: `High-level design`, body: `<p>Event ingest → router by preference → per-channel workers (push/email/SMS) → delivery log for idempotency. Template service separate.</p>`, figureAfter: "architecture" },
    { title: `Deep dives`, body: `<p>Pick 2–3 areas interviewers probe: data model, partitioning, caching, consistency. For <b>Notification System</b>, explain id generation, hot keys, and what fails under 10× load.</p>
