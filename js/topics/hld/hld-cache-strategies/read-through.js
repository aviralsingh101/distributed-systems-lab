...
<p>Distinguish <b>Read Through</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Cache loads DB itself.</p><p>Trace one end-to-end request through <b>Read Through</b>: what triggers it, which components participate, where state is stored, and what the client observes on success vs timeout.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p><p>Detail read path (cache hit/miss), write path (invalidate vs write-through), and TTL as staleness backstop. What happens on cache cluster partition?</p>`, figureAfter: "cache-flow" },
    { title: `HLD placement`, body: `<p><b>Read Through</b> sits between application and origin store. Draw cache hit/miss paths and invalidation triggers.</p>` },
...
<p><b>Failure isolation:</b> If <b>Read Through</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
