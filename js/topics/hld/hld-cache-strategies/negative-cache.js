...
<p>Distinguish <b>Negative Cache</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Cache 'not found'.</p><p>Document sync vs async usage: blocking calls belong on the critical user path only when necessary; everything else should be queued or event-driven with clear compensation.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p><p>Detail read path (cache hit/miss), write path (invalidate vs write-through), and TTL as staleness backstop. What happens on cache cluster partition?</p>`, figureAfter: "cache-flow" },
    { title: `HLD placement`, body: `<p><b>Negative Cache</b> sits between application and origin store. Draw cache hit/miss paths and invalidation triggers.</p>` },
...
<p><b>Failure isolation:</b> If <b>Negative Cache</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
