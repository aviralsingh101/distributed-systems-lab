...
<p>Distinguish <b>TLS Termination</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Where encryption ends: edge vs service.</p><p>Trace one end-to-end request through <b>TLS Termination</b>: what triggers it, which components participate, where state is stored, and what the client observes on success vs timeout.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p><p>Specify wire protocol, connection lifecycle, head-of-line blocking risks, and whether intermediaries (proxies, LBs) terminate or pass through. State default timeouts and retry idempotency requirements.</p>`, figureAfter: "request-flow" },
    { title: `HLD placement`, body: `<p>On the architecture diagram, <b>TLS Termination</b> sits on the communication path between clients and services — or between services. Label connection type (sync/async), timeout budget, and retry policy on the arrow.</p><p>Show where TLS terminates and whether connections are pooled or per-request. Interviewers often ask how this choice affects tail latency under packet loss.</p>` },
...
<p><b>Failure isolation:</b> If <b>TLS Termination</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
