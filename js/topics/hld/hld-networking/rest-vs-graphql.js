...
<p>Interview tip: whiteboard the charge flow, mark where <b>REST vs GraphQL</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ...
  figures: [
    { id: "comparison", svg: `<svg viewBox="0 0 480 150" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="REST vs GraphQL comparison">
...
<text x="240" y="30" text-anchor="middle" fill="#cdd6e8" font-size="12" font-weight="600" font-family="system-ui">REST vs GraphQL</text>
...
<text x="240" y="135" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">Prototype under realistic load before choosing</text>
</svg>`, caption: `Decision fork for REST vs GraphQL — weigh simplicity vs scale before committing.` }
  ],
  related: [],
  ...
  template: "tradeoff",
  sim: () => ({
    note: `Explore REST vs GraphQL in the payment platform.`,
    toggleLabel: "Switch approach",
    labelA: "Without pattern",
    labelB: "With REST vs GraphQL",
    sideA: () => ({ nodes: [
    ...
    sideB: () => ({ nodes: [
      ...
      { title: "REST vs GraphQL", value: "applied" },
      { title: "Independent scale", value: "ok" },
    ]}),
    status: (ctx, t, useB) => ({ text: useB ? "REST vs GraphQL — better fit" : "naive — hits limits", cls: useB ? "ok" : "warn" }),
  }),
});
