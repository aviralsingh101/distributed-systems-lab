...
<p>Distinguish <b>False Sharing</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>CPU cache line contention.</p><p>Define SLIs affected by <b>False Sharing</b> — latency, availability, error rate — and which dependency failure degrades each.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p>`, figureAfter: "request-flow" },
    { title: `HLD placement`, body: `<p><b>False Sharing</b> affects queueing and latency on the critical path — annotate p99 budget and backpressure points.</p>` },
...
<p><b>Failure isolation:</b> If <b>False Sharing</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
...
export function createSimulation(stage, panel, stageEl) {
  return createTopicSim("false-sharing", stage, panel, stageEl);
}