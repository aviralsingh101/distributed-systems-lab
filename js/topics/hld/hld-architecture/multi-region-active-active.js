...
<p>Distinguish <b>Multi-Region Active-Active</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Serve from multiple regions live.</p><p>Document sync vs async usage: blocking calls belong on the critical user path only when necessary; everything else should be queued or event-driven with clear compensation.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p>`, figureAfter: "architecture" },
    { title: `HLD placement`, body: `<p><b>Multi-Region Active-Active</b> shapes service boundaries and event flows. Draw services as boxes; show command vs query paths and async boundaries explicitly.</p><p>Each box should own one write model; cross-box updates go through events or sagas.</p>` },
...
<p><b>Failure isolation:</b> If <b>Multi-Region Active-Active</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
