...
<p>Distinguish <b>Serverless / FaaS</b> from adjacent concepts in the sidebar — interviewers test whether you know boundaries, not buzzword definitions.</p>` },
    { title: `How it works`, body: `<p>Functions triggered by events.</p><p>Trace one end-to-end request through <b>Serverless / FaaS</b>: what triggers it, which components participate, where state is stored, and what the client observes on success vs timeout.</p><p>Walk through the happy path first, then one degraded path (slow dependency, partial outage, stale cache).</p>`, figureAfter: "architecture" },
    { title: `HLD placement`, body: `<p><b>Serverless / FaaS</b> shapes service boundaries and event flows. Draw services as boxes; show command vs query paths and async boundaries explicitly.</p><p>Each box should own one write model; cross-box updates go through events or sagas.</p>` },
...
<p><b>Failure isolation:</b> If <b>Serverless / FaaS</b> fails, what still works? Define blast radius and graceful degradation — read-only mode, cached responses, or queue for later.</p>
