...
const topic = makeTopic({
  ...
  sections: [
    { title: `What is Cell Architecture (design)?`, body: `<p><b>Cell Architecture (design)</b> — Isolated failure domains at scale.</p>
<p>In the payment platform topology, <b>Cell Architecture (design)</b> sits on the request or data path between Client/Order and shared infrastructure (Gateway, Ledger, Queue). Draw it explicitly on architecture diagrams with failure domains marked.</p>
<p>Unlike a generic "best practice" label, it has a specific seat in the payment platform architecture with defined inputs, outputs, and failure modes.</p>` },
    { title: `How it works`, body: `<p>At runtime, components interact in a defined order with configurable timeouts, health checks, and backoff policies. Trace a single <code>POST /v1/charge</code> with <code>X-Request-ID</code> to see where <b>Cell Architecture (design)</b> applies.</p>
...
<p>Capacity planning: measure peak QPS, payload size, and fan-out before scaling horizontally. Tail latency (p99) often reveals misconfiguration before mean latency moves.</p>` },
    { title: `In production`, body: `<p>Operate with dashboards: error rate, p99 latency, saturation, and dependency health. Runbooks cover failover, key rotation, and rollback. Game-day exercises validate that <b>Cell Architecture (design)</b> behaves correctly during AZ failure or broker restart.</p>
<p>Security: TLS on public paths; no PAN in application logs; audit admin APIs that change <b>Cell Architecture (design)</b> configuration.</p>` },
...
<li>Operating without correlation IDs across Order → Gateway → Ledger.</li>
    ...
    { title: `Production checklist`, body: `<p>Before shipping <b>Cell Architecture (design)</b> changes to production:</p>
<ul>
