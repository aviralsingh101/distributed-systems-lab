...
export const content = {
  oneliner: `SET key uuid NX PX; own the token.`,
  archetype: "pattern",
  sections: [
    { title: `Motivation`, body: `<p>SET key uuid NX PX; own the token.</p>
<p>Without <b>Redis Lock</b>, Order Service code accrues ad-hoc fixes — duplicate event handlers, tangled dependencies, and untestable static calls that break under parallel payment load.</p>` },
...
<p>Map the pattern to packages: domain interfaces, infrastructure adapters, and thin HTTP handlers. Unit tests use fakes; integration tests use Testcontainers for Postgres and Kafka.</p>` },
    { title: `Implementation flow`, body: `<p>Typical charge flow with <b>Redis Lock</b>:</p>
<ol>
...
<p><b>Use when:</b> the problem shape matches what <b>Redis Lock</b> was designed for and simpler code is failing reviews or incidents.</p>` },
    { title: `Production checklist`, body: `<p>Before shipping <b>Redis Lock</b> changes to production:</p>
<ul>
...
<p>Interview tip: whiteboard the charge flow, mark where <b>Redis Lock</b> applies, and describe one real failure mode and its fix with concrete SQL or config.</p>` }
  ],
  figures: [
...
<text x="200" y="22" text-anchor="middle" fill="#93a1bd" font-size="10" font-family="system-ui">Only token holder may write</text>
  ...
  related: ["distributed-lock", "fencing-token", "leader-election"],
};

export function createSimulation(stage, panel, stageEl) {
  return sequenceSim(stage, panel, stageEl, {
    note: "Two workers, one Redis lock key.",
    toggles: [{ key: "fix", label: "Release with ownership check (Lua CAS)", kind: "ok", value: false }],
    scenario(ctx) {
      const fix = ctx.toggles.fix;
      const actors = [
        { id: "a", label: "Worker A", color: C.service },
        { id: "r", label: "Redis lock", color: C.gateway, kind: "db", value: "free" },
        { id: "b", label: "Worker B", color: C.ledger },
      ];
      const steps = [
        { from: "a", to: "r", label: "SET NX uuid=aaa ✓", good: true, set: { r: "aaa", a: "holds" } },
        { from: "a", to: "a", label: "slow work > TTL", self: true, set: { a: "still running" } },
        { from: "r", to: "r", label: "TTL expires", self: true, set: { r: "free" } },
        { from: "b", to: "r", label: "SET NX uuid=bbb ✓", good: true, set: { r: "bbb", b: "holds" } },
        fix
          ? { from: "a", to: "r", label: "DEL if val=aaa → no-op", good: true, dashed: true, set: { a: "not owner" } }
          : { from: "a", to: "r", label: "DEL (blind)", bad: true, set: { r: "free", b: "lock stolen!" } },
      ];
      return {
        actors, steps, stepDur: 1.1,
        status: (r) => !r.done ? { text: "A slow past TTL…", cls: "" }
          : fix ? { text: "B keeps its lock (owner-checked release)", cls: "ok" } : { text: "A deleted B's lock — mutual exclusion broken", cls: "err" },
      };
    },
  });
}
